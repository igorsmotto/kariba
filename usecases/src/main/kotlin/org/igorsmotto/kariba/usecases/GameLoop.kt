...
class GameLoop(
    ...
    fun execute(startingGameState: GameState): PlayerState {
        val players = startingGameState.players.map { Player(it.name, it.hand, it.points) }
        val lake = Lake(startingGameState.lake.water)
        val
    ...
    private fun validPlay(play: Play, player: Player): Boolean {
        val cardInPlay = Card.valueOf(play.card)
